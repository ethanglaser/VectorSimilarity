...
static void FP32_normalizeSIMD16_AVX(void *pVect1v, size_t dimension) {
    ...
    if constexpr (residual % 8) {
        /*
        Why is that?
        Without the blend, the part over the residual is effected by the store_eu
        And is mul/div by norm factor twice.
        There might be a better way to do this, but this is not a primary focus now.
        */
        __mmask8 constexpr mask8 = (1 << (residual % 8)) - 1;
        __m256 v1 = _mm256_loadu_ps(pVect1);
